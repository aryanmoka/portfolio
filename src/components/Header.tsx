...
import { Menu, X, Home, User, Cpu, Layers, Award, Mail, Briefcase, FileText } from 'lucide-react';
...
const Header: React.FC = () => {
  ...
  // 2. Active Section Spy — re-runs whenever we land back on the home route,
  // since the sections don't exist in the DOM while on a case study page.
  useEffect(() => {
    if (!isHome) return;
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    if (!isHome) {
      // Navigate back to home, then scroll once the sections mount.
      navigate(`/${href}`);
      return;
    }
              ...
              <div className="p-4 space-y-2">
                ...
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
                    path === '/resume'
                      ? 'bg-gradient-to-r from-cyan-500/20 to-transparent text-white border-l-2 border-cyan-500'
                      : 'text-cyan-400 hover:bg-white/5 hover:text-white'
                  }`}
                >
                  <FileText size={16} />
